...
watch: requires_nix_shell
	while sleep 1; do find src hedgehog-plutus-simple.cabal | entr -cd make build; done
...
ghci: requires_nix_shell
	cabal v2-repl $(GHC_FLAGS) hedgehog-plutus-simple
...
build_path = dist-newstyle/build/x86_64-linux/ghc-8.10.4.20210212/hedgehog-plutus-simple-0.1
clear_build:
